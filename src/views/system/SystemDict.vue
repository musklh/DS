      ...
      <div class="main-content">
        <div class="dict-header">
          <div class="button-group">
            ...
            <!-- 添加下载模板按钮 -->
            <el-button type="primary" plain :disabled="uploading" @click="downloadTemplate">
        ...
        >
          ...
          <el-table-column prop="word_class" label="词条类型" width="120">
            <template #default="{ row }">
              <el-tag :type="getTagType(row.word_class)">
                {{ row.word_class }}
              </el-tag>
            </template>
          </el-table-column>
        ...
        <el-dialog v-model="importResultVisible" title="导入结果" width="500px">
          ...
          <template #footer>
            <span class="dialog-footer">
              <el-button v-if="!uploading" @click="importResultVisible = false">关闭</el-button>
              <el-button
                v-if="!uploading && importResult.success"
                type="primary"
                @click="refreshAfterImport"
              >
                确定并刷新
              </el-button>
            </span>
  ...
  setup() {
    ...
    const handleBatchDelete = async () => {
      ...
      try {
        ...
        await fetchDictList();
        ElMessage.success(
          `成功删除 ${successCount} 条记录` + (failCount > 0 ? `，${failCount} 条记录删除失败` : '')
        );
