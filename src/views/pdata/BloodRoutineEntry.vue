                ...
                <el-checkbox-group v-model="formData.values[item.word_code].selected">
                  <div v-for="option in item.options.split(',')" :key="option" class="checkbox-time-item">
                    ...
                    <div v-if="isOptionSelected(item.word_code, option) && item.followup_options && item.followup_options[option]">
                      ...
                      <!-- RENDER GROUPED FOLLOWUPS -->
                      <div v-if="item.followup_options[option].input_type === 'group'" class="followup-group-container">
                        <div v-for="field in item.followup_options[option].fields" :key="field.label" class="followup-group-item">
                            <span class="followup-label">{{ field.label }}:</span>
                            <el-select 
                                v-if="field.input_type === 'select'" 
                                v-model="formData.values[item.word_code].followup[option][field.label]" 
                                placeholder="请选择" 
                                size="small" 
                                style="width: 150px;"
                            >
                                <el-option v-for="opt in getOptionsArray(field.options)" :key="opt" :label="opt" :value="opt" />
                            </el-select>
                            <el-date-picker 
                      ...
                      <!-- RENDER SINGLE/NESTED FOLLOWUPS (existing logic) -->
                      <div v-else>
...
// 确保为 'group' 类型的 followup 初始化对象
watch(
  () => formData.values,
  (newValues) => {
    if (!props.selectedTemplate?.dictionaryList) return;

    for (const word_code in newValues) {
      const current = newValues[word_code];
      const item = props.selectedTemplate.dictionaryList.find(i => i.word_code === word_code);

      if (item && (item.input_type === 'multi' || item.input_type === 'multi_with_date') && current.selected) {
        current.selected.forEach(option => {
          const followup_config = item.followup_options?.[option];
          if (followup_config?.input_type === 'group') {
            if (typeof current.followup[option] !== 'object' || current.followup[option] === null) {
              current.followup[option] = {};
            }
          }
        });
      }
