...
// 获取计算属性列表（带分页）
export const fetchCalculationList = async (
  page: number,
  size: number,
  keyword: string = ''
): Promise<{ data: CalculationAttribute[]; total: number }> => {
  try {
...
// 添加计算属性
export const addCalculationAttribute = async (
  item: CalculationAttribute
): Promise<CalculationAttribute> => {
  try {
...
// 更新计算属性
export const updateCalculationAttribute = async (
  id: number,
  item: CalculationAttribute
): Promise<CalculationAttribute> => {
  try {
